...
from utils.checks import check_imgsz, check_requirements, check_version
...
class Exporter:
    ...
    def __call__(self, model=None):
        ...
        if format in (
            ...
            os.environ["PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION"] = (
                "python"  # fix attempt for protobuf<3.20.x errors
            )
            format = "coreml"
        ...
        if any(f):
            ...
            predict_data = (
                f"data={data}" if model.task == "segment" and format == "pb" else ""
            )
            q = (
    ...
    def export_ncnn(self, prefix=colorstr("ncnn:")):
        ...
        if Path(pnnx_filename).is_file():
            ...
            system = (
                "macos" if MACOS else "ubuntu" if LINUX else "windows"
            )  # operating system
            asset = (
                [x for x in assets if system in x][0]
                if assets
                else f"https://github.com/pnnx/pnnx/releases/download/20230816/pnnx-20230816-{system}.zip"
            )  # fallback
    ...
    def export_coreml(self, prefix=colorstr("CoreML:")):
        ...
        check_requirements(
            "coremltools>=6.0,<=6.2" if mlmodel else "coremltools>=7.0.b1"
        )
        ...
        if self.args.nms and self.model.task == "detect":
            if mlmodel:
                ...
                check_version(
                    platform.python_version(), "<3.11", name="Python ", hard=True
                )
                weights_dir = None
            else:
