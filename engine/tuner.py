...
from utils import DEFAULT_CFG, LOGGER, callbacks, colorstr, yaml_print, yaml_save
...
class Tuner:
    ...
    def __call__(self, model=None, iterations=10, prefix=colorstr("Tuner:")):
        ...
        for i in range(iterations):
            ...
            LOGGER.info(
                f"{prefix} Starting iteration {i + 1}/{iterations} with hyperparameters: {mutated_hyp}"
            )
            ...
            try:
                ...
                LOGGER.warning(
                    f"WARNING ❌️ training failure for hyperparameter tuning iteration {i}\n{e}"
                )
                fitness = 0.0
            ...
            headers = (
                ...
                else (",".join(["fitness_score"] + list(self.space.keys())) + "\n")
            )
            ...
            header = (
                f'{prefix} {i + 1} iterations complete ✅ ({time.time() - t0:.2f}s)\n'
            ...
            header = (
                header.replace(prefix, "#").replace("[1m/", "").replace("[0m", "")
                + "\n"
            )
