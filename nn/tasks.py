...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Conv, ConvTranspose, GhostConv, Bottleneck, SPPF, DWConv, Focus, MSBlockLayer, MSBlock, Res2Net,
                 C2f, nn.ConvTranspose2d, DWConvTranspose2d, GSBottleneck, GSBottleneckC, VoVGSCSP, VoVGSCSPC, CPS_A,
                R2NFN , FasterNetLayer, PConv, GSConv):
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)

            args = [c1, c2, *args[1:]]
            if m in (C2f, MSBlock, Res2Net, R2NFN):
